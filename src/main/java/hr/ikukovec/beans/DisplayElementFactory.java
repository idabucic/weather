...
public class DisplayElementFactory {
	 ...
	 */
	public static DisplayElement createDisplayElement(DisplayElementType det) {
		...
		if (det.getType() == DisplayElementType.CURRENT_WEATHER.getType()) {
			...
			//this bean and it's view will not be used independently
			String xhtmlFile = null;
			de = new WeatherForecastBean(det, xhtmlFile);
		} else if (det.getType() == DisplayElementType.FORECAST.getType()){
			String xhtmlFile = "forecast.xhtml";
			de = new ForecastBean(det, xhtmlFile);
